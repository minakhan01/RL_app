...
const BreakScreen = () => {
  ...
  if (breakState === "break" && !minimized) {
    ...
    curWindow.setSize(electron.remote.screen.getPrimaryDisplay().size.width, Math.ceil(electron.remote.screen.getPrimaryDisplay().size.height))
    curWindow.setPosition(0, 0)

    return <div className="break-div">
      <ToolBar type='minimize' minimize={setMinimized} totaltime={breakDuration} startTime={breakStartTime} />
      <div className="break-text">
        Be kind!
      </div>
      <div className="countdown-timer">
        <Timer totaltime={breakDuration} startTime={breakStartTime} />
      </div>
    </div>
  }
    ...
    curWindow.unmaximize()
    curWindow.setSize(electron.remote.screen.getPrimaryDisplay().size.width, Math.ceil(electron.remote.screen.getPrimaryDisplay().size.height / 8))
    curWindow.setPosition(0, (7 / 8) * electron.remote.screen.getPrimaryDisplay().size.height)
